...
const wsRegistryPLugin = fp(async function (fastify: FastifyInstance): Promise<void> {
  ...
  fastify.decorate('wsRegistry', {
    ...
    process: async (message: wsApi.WSMessage, socket: WebSocket, request: FastifyRequest) => {
      ...
      const validation = fastify.validateWSMessage(
        message.params,
        entry.schema.request
      );
      if (!validation.valid) {
        return {
          id: message.id,
      ...
      try {
        ...
        let responseValidation: { valid: boolean; errors?: any; data?: any } = { valid: false, errors: [], data: {} };
        /**
        ...
        responseValidation = fastify.validateWSResponse(
          result,
